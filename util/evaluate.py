...
def evaluate_raw(gold_entities: List[List[Tuple[int, int, int]]], pred_entities: List[List[Tuple[int, int, int]]]) \
        -> Tuple[int, int, int]:
    
    # Entity = (Start pos, End pos, Tag id)
...
def evaluate_raw_tagged(gold_entities: List[List[Tuple[int, int, int]]], pred_entities: List[List[Tuple[int, int, int]]], tags_alphabet : Alphabet):
    
    import copy
    counts = copy.deepcopy(dict())
    
    for tag in tags_alphabet.instances:
        counts[tag] = copy.deepcopy({"tp" : 0, "fp" : 0, "fn" : 0})
    ...
    for g_ets, p_ets in zip(gold_entities, pred_entities):
        ...
        for et in g_ets:
            tag_id = et[2]
            if et in p_ets:
                counts[tags_alphabet.get_instance(tag_id)]['tp'] += 1
            else:
                counts[tags_alphabet.get_instance(tag_id)]['fn'] += 1
        for et in p_ets:
            tag_id = et[2]
            if et not in g_ets:
                counts[tags_alphabet.get_instance(tag_id)]['fp'] += 1
